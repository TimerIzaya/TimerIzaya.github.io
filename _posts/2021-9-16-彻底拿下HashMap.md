...
2. 对于整体table，有几个关键的变量需要知道。
   ...
   2. #### 升级红黑树
      ...
      这里其实不管λ是多少，比如0.6、0.7，泊松分布所计算得进入同个桶8次的概率都低的离谱，为千万分之一。这里贴一下大概的表。源码注释中默认λ是0.5。
...
​			他说λ为0.5是因为threashold是0.75，取一个大概的值，具体0.5怎么来的没说明白，网上众说纷纭，下面我来说一下我的理解和			证明。
...
​			作者只带入了扩容一次的λ，事实上，以f为0.75举例，当λ接近0.375的时候，bin里链表的长度为8的概率比0.5还要小六倍，数据			可见上表格。这也解释了f为什么越小，越不容易冲突。
