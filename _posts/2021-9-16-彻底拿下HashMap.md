...
2. ### 扩容机制
   ...
   - **hashmap也不是链表长度小于6就会转红黑树，而是在resize的时候对红黑树进行split操作，如果长度小于6，才会退化。**
...
3. ### 线程安全性
   ...
   首先这是个很无聊的问题，一个设计之初本就没有考虑线程安全的数据结构，与其去问为什么它线程不安全，不如去问它的线程安全版本ConcurrentHashMap是怎么实现的。**（真几把脑瘫，问这种问题的面试官脑子指定有屎）**
