...
![](https://raw.githubusercontent.com/TimerIzaya/TimerBlogPic/master/Package util.png)

Util里主要容器类就是Collection和Map，先研究Map这一块，Map容器系列设计结构如下。

![](https://raw.githubusercontent.com/TimerIzaya/TimerBlogPic/master/20210916153537.png)
...
![](https://raw.githubusercontent.com/TimerIzaya/TimerBlogPic/master/20210916162353.png)
...
2. 对于整体table，有几个关键的变量需要知道。
   ...
   - #### 升级红黑树
     ...
     ​                                          <img src="https://raw.githubusercontent.com/TimerIzaya/TimerBlogPic/master/image-20211115201341692.png" alt="image-20211115201341692" style="zoom:50%;" /> 
...
1. ### 哈希算法
   ...
   之前说了为什么table选取2的n次方，原因之一就是这里优化的table的索引计算。一般来说，当算出key的hash值后，会对数组长度	进行mod操作，而mod底层又是无限除法，性能较差。**这里是对hash值和(table.length - 1) 进行 & 操作**。
...
2. ### 扩容机制
   ...
   - ### 链表迁移
     ...
     稍微解释一下，如果length为16，那么意味着e.hash只关注后四位，如果length为32，那么意味着e.hash只关注后五位。
     ...
     if (loTail != null) {
         loTail.next = null;
         newTab[j] = loHead; //lo不变
     }
     if (hiTail != null) {
         hiTail.next = null;
         newTab[j + oldCap] = hiHead; //hi变
     }
     ```
...
ConcurrnetHashMap 由很多个 Segment 组合，而每一个 Segment 是一个类似于 HashMap 的结构，所以每一个 HashMap 的内部可以进行扩容。但是 Segment 的个数一旦**初始化就不能改变**，默认 Segment 的个数是 16 个，你也可以认为 ConcurrentHashMap 默认支持最多 16 个线程并发。由于 Segment 继承了 ReentrantLock，所以 Segment 内部可以很方便的获取锁，put 流程就用到了这个功能。
